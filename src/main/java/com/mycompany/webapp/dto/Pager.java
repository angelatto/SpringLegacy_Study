...
public class Pager {
	...
	private int totalGroupNo;	//전체 그룹 수 == 12345 한번에 보여지는 페이지 번호들 
	...
	public Pager(int rowsPerPage, int pagesPerGroup, int totalRows, int pageNo) {
		...
		totalPageNo = totalRows / rowsPerPage;  /* 전체 행수 나누기 페이지당 행수 == 페이지 개수  */
		if(totalRows % rowsPerPage != 0) totalPageNo++; // 딱 떨어지지 않으면 페이지수 + 1 
		
		totalGroupNo = totalPageNo / pagesPerGroup; /* 전체 페이지 수 나누기 그룹당 페이지 개수 == 그룹의 개수  */
		if(totalPageNo % pagesPerGroup != 0) totalGroupNo++;
		
		groupNo = (pageNo - 1) / pagesPerGroup + 1; /* 현재 페이지가 속한 그룹 == */
		
		startPageNo = (groupNo-1) * pagesPerGroup + 1; /*  그룹의 시작 번호 */
		
		endPageNo = startPageNo + pagesPerGroup - 1; /*  그룹의 끝 번호 */
		if(groupNo == totalGroupNo) endPageNo = totalPageNo; /* 마지막 그룹일 경우 마지막 페이지 번호가 끝 번호이다.*/
